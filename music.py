...
async def 컴(ctx):
    try:
        ...
        try:
            ...
            await ctx.send("노래를 듣기전에 음성채널에 먼저 접속해주세요.")
...
async def 재생(ctx, *, msg):
    if not vc.is_playing():
        ...
        driver_dir = r"C:\Users\Administrator\Desktop\driver.exe"
        driver = webdriver.driver_dir
...
async def 멜론차트(ctx):
    if not vc.is_playing():
        ...
        driver_dir = r"D:\Discord_Bot\driver.exe"
        driver = webdriver.driver_dir, options = options
...
\n!컴 -> 뮤직봇을 자신이 속한 채널로 부릅니다.
\n!아웃 -> 뮤직봇을 자신이 속한 채널에서 내보냅니다.
...
bot.run('ODc0NTE2ODI1NTYyNTU0NDA5.YRIHQg.4jHKTC5tTA4iSaXXzVNkxhV6tlw')